...
def generate_cluster(cluster_nodes, k):
    ...
    while i <= k:
        ...
        for v in processed_nodes:
            ...
            int_deg[u] = max(0, int_deg[u] - 1)
            int_deg[v] = max(0, int_deg[v] - 1)
            int_probs[node2cluster[u], node2cluster[v]] = max(
                0, int_probs[node2cluster[u], node2cluster[v]] - 1)
            int_probs[node2cluster[v], node2cluster[u]] = max(
                0, int_probs[node2cluster[v], node2cluster[u]] - 1)
