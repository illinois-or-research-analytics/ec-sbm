...
for file in S2_files:
    ...
    r = fn.split('-')[1].split('_')[0][1:]

    if r not in RS:
        continue

    print('S2', network_id, r)
    output_folder = f'{output_root}/leiden_cpm/{network_id}/leiden{r}/'
    os.makedirs(output_folder, exist_ok=True)
...
for file in S5_files:
    ...
    network_id = fn.split('-')[0].replace('S5_', '')
    ...
    output_folder = f'{output_root}/leiden_cpm_cm/{network_id}/leiden{r}/'
    os.makedirs(output_folder, exist_ok=True)
    os.system(f'cp {edgelist_files[network_id]} {output_folder}/edge.dat')
    os.system(f'cp {file} {output_folder}/com.dat')
...
for network_id in NETWORK_IDS:
    for r in RS:
        if not os.path.exists(f'{output_root}/leiden_cpm/{network_id}/leiden{r}/edge.dat'):
        ...
        if not os.path.exists(f'{output_root}/leiden_cpm_cm/{network_id}/leiden{r}/edge.dat'):
            print(f'{network_id} {r} S1/S5 not found')

        if not os.path.exists(f'{output_root}/leiden_cpm_cm/{network_id}/leiden{r}/com.dat'):
            print(f'{network_id} {r} S5 not found')
