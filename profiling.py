...
out_fp = Path(
    f'output/profiles/time_{split}_{method}_{clustering}_{resolution}.csv')
...
for network_id in network_ids:
    ...
    profile_mapping = {
        'e_repl_cluster': 'INFO - Replicate clustering: ',
        'e_combine': 'INFO - Combine clustered and outlier subgraphs: ',
    }
...
df['o_total'] = \
    ...
    df['o_repl_cluster']
df['e_total'] = \
    df['e_process_cluster'] + \
    df['e_process_edgelist'] + \
    df['e_create_outlier_cluster'] + \
    df['e_compute_sbm_original'] + \
    df['e_update_sbm_existing'] + \
    df['e_add_edge'] + \
    df['e_post_process'] + \
    df['e_combine'] + \
    df['e_repl_cluster']
