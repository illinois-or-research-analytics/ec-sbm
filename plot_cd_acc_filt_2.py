    ...
    for network_id in network_ids:
        for cd_clustering, cd_resolution in cd_clusterings_resolutions:
            ...
            acc_root = root / method / gt_clustering / network_id / \
                gt_resolution / '0' / cd_clustering / cd_resolution

            for suffix, stage in m.items():
                acc_fp = acc_root / suffix / 'accuracy.txt'
...
for split in ['val_large', 'val_medium', 'val_small']:
    ...
    output_root = Path(
        f'output/comdet_acc/{split}/{method}/{gt_clustering}/{gt_resolution}/filt')
