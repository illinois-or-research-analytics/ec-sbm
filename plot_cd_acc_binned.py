...
parser.add_argument('--stat', type=str,
                    default='mixing_xi', help='Stat name')
...
networks_lists = [
    x
    for x in Path(f'output/stats/sbmmcs+e/{gt_clustering}_{gt_resolution}/bin/{stat}').iterdir()
    if x.is_file() and x.name.endswith('.txt')
]

splits = [
    x.stem
    for x in networks_lists
]
...
dfs = dict()
for networks_list, split in zip(networks_lists, splits):
    root = Path(f'data/comdet_acc/')

    output_root = Path(
        f'output/comdet_acc/bin/{method}/{gt_clustering}/{gt_resolution}/{stat}/{split}/all')
...
# Combine all dfs, making a new column called 'split'
dfs_combined = pd.concat(
    [df.assign(split=split) for split, df in dfs.items()]
)
dfs_combined = dfs_combined.sort_values(
    'split', key=lambda x: x.str.extract(r'\((.*), (.*)\]').astype(float).apply(tuple, axis=1)
)
dfs_combined.to_csv(
    Path(
        f'output/comdet_acc/bin/{method}/{gt_clustering}/{gt_resolution}/{stat}/all_table.csv'),
    index=False,
)
...
fig.savefig(
    Path(
        f'output/comdet_acc/bin/{method}/{gt_clustering}/{gt_resolution}/{stat}/all_boxplot.pdf'),
    bbox_inches='tight',
)
